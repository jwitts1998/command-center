...
export async function listProjects(): Promise<void> {
  ...
  try {
    ...
    console.log(
      chalk.dim(
        `  ${chalk.green(`${summary.healthy} healthy`)} · ${chalk.yellow(`${summary.warning} warning`)} · ${chalk.red(`${summary.error} error`)} · ${chalk.gray(`${summary.unknown} unknown`)}\n`
      )
    );
    ...
    console.log(
      `  ${chalk.bold("Name".padEnd(25))}${chalk.bold("Phase".padEnd(15))}${chalk.bold("Health".padEnd(10))}`
    );
    console.log(chalk.dim("─".repeat(60)));
    ...
    for (const repo of status.repos) {
      ...
      console.log(
        `  ${repo.name.padEnd(25)}${(repo.status.phase || "unknown").padEnd(15)}${healthColor(repo.status.health.padEnd(10))}`
      );
