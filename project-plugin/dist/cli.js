...
async function setup(options) {
    ...
    // Find project root
    const projectRoot = (0, config_js_1.findProjectRoot)();
    if (!projectRoot) {
    ...
    // Get existing config
    const existingConfig = (0, config_js_1.loadConfig)(projectRoot || undefined);
    // Prompt for values
    const commandCenterUrl = options.url ||
        (await prompt(`Command Center URL [${existingConfig.commandCenterUrl}]: `)) ||
    ...
    (0, config_js_1.saveConfig)(config, projectRoot || undefined);
    console.log(colors.green('\n✓ Configuration saved'));
    console.log(colors.dim(`  Config: ${(0, config_js_1.getConfigDir)(projectRoot || undefined)}/config.json`));
    // Test connection
    console.log('\nTesting connection...');
    const plugin = (0, plugin_js_1.createPlugin)(projectRoot || undefined);
...
async function status() {
    ...
    const projectRoot = (0, config_js_1.findProjectRoot)();
    const configured = (0, config_js_1.isConfigured)(projectRoot || undefined);
    console.log(`Configuration: ${configured ? colors.green('Found') : colors.yellow('Not configured')}`);
    if (configured) {
        const config = (0, config_js_1.loadConfig)(projectRoot || undefined);
    ...
    console.log('\nProject Context:');
    const context = (0, config_js_1.loadProjectContext)(projectRoot || undefined);
 ...
 */
async function enrich(promptText, options) {
    if (!(0, config_js_1.isConfigured)()) {
