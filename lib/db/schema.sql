...
CREATE TABLE IF NOT EXISTS notifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id VARCHAR(255),                     -- For future multi-user support
  type VARCHAR(50) NOT NULL,                -- approval_required, task_assigned, agent_status_change, budget_alert, goal_completed
  title VARCHAR(255) NOT NULL,
  message TEXT,
  metadata JSONB DEFAULT '{}',              -- Additional context (entity_id, entity_type, etc.)
  is_read BOOLEAN DEFAULT false,
  read_at TIMESTAMP WITH TIME ZONE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  expires_at TIMESTAMP WITH TIME ZONE       -- Optional expiration for transient notifications
);
